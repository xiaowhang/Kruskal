...
function getRandomInt(min: number, max: number): number { // 获取一个随机整数
  return Math.floor(Math.random() * (max - min + 1)) + min;
}

function createNode(id: number, x: number, y: number): HTMLDivElement { // 创建节点元素
...
function checkDistance(nodes: { x: number, y: number }[], x: number, y: number): boolean { // 检查节点之间的距离
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  refreshButton.addEventListener('click', function refreshPage(): void { location.reload(); }); // 添加点击事件处理程序
...
// 点击时的处理
function edgeClickHandler(event: MouseEvent) {
...
function KruskalAlgorithm(nodes: { id: number, x: number, y: number }[], edges: { from: number, to: number, weight: number }[]): void {
  // 按边的权值升序排序
...
function showCorrectAnswer(): void {
  ...
  for (let i = 0; i < edges.length; i++) {
    const edge = edges[i] as HTMLDivElement;
